...
export function apply(ctx) {
  ...
  async function warmSession(session) {
    ...
    try {
      ...
      // alpha.5 起 identity 含 isSeeded/inheritedEventCount（identityMatches 四字段全等），
      // 直接传 header 不再匹配——按 identityOf 语义构造。
      const header = session.header
      const identity = {
        createdAt: header?.createdAt,
        ...(header?.cwd !== void 0 ? { cwd: header.cwd } : {}),
        isSeeded: header?.isSeeded ?? false,
        inheritedEventCount: sessionLogOffset(session.inheritedEventCount ?? 0),
      }
      const record = cache && typeof cache.recordFor === 'function'
        ? cache.recordFor(session.id, identity) : void 0
  ...
  async function backfillColdSessionsInner() {
    ...
    // alpha.5：putSoft 被 put(id, identity, rows) 取代；兼容两者
    ...
    for (const cand of candidates.slice(0, config.backfillMaxSessions)) {
      try {
        ...
        // alpha.5 identity：isSeeded/inheritedEventCount 参与匹配；冷日志的
        // header 行按 toHeaderLine 语义携带 isSeeded/seedLength
        const existing = cache.recordFor?.(cand.id, {
        ...
        await putCacheRow.call(cache, cand.id, identity, rows)
        stats.backfilled += 1
