...
// rc.6 源码特征校验，结构不符自动跳过；三层回退（能力探测/try-catch/
// 配置开关）；dispose 完整还原。
...
export function apply(ctx) {
  ...
  try {
    ...
    if (settings && typeof settings.register === 'function' && schemaLib) {
      ...
      if (resolved && typeof resolved === 'object') Object.assign(config, resolved)
    }
  } catch (error) {
  ...
  // 5×700MB 叠加是 OOM 主因之一。运行时调低 capacity 并淘汰最旧 ready entry，
  // 主动释放冷会话事件树。
  function installPreparedCacheTrim(ictx) {
    ...
    const target = config.preparedCacheSize
  ...
  async function installChunkedMaterialize(ictx) {
    ...
    const src = String(persistence.encodeMaterialization)
    ...
    const original = persistence.encodeMaterialization.bind(persistence)
    ...
    disposers.push(() => { if (persistence.encodeMaterialization !== original) persistence.encodeMaterialization = original })
  ...
  async function backfillColdSessions() {
    ...
    const sessionsRoot = (() => {
      try {
        ...
        return paths?.sessions?.() ?? paths?.dshHome?.() + '/sessions'
      } catch { return void 0 }
        ...
        try {
          ...
          if (method === 'config.set') {
            ...
            for (const key of Object.keys(DEFAULT_CONFIG)) {
              if (payload && key in payload && typeof payload[key] === typeof config[key]) config[key] = payload[key]
            }
            try {
              const settings = ctx.get('settings')
