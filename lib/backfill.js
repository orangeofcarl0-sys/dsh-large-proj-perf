...
export function createBackfill(pc) {
  ...
  // 按 cache 的 identityOf 语义构造匹配键：isSeeded/inheritedEventCount
  //（alpha.5 起）+ formatVersion（0.1.3 起 identityMatches 要求
  // formatVersion === header.version；缺它的行只能当 title 前驱提示，
  // 不能作折叠短路——补行必须带上才能被打开路径采用）。
